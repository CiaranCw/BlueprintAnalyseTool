...
UK2Node_CallFunction* FBPGen::SpawnCallArrayFunc(UEdGraph* G, UClass* OwnerClass, FName FuncName, int32 X, int32 Y)
{
	if (!OwnerClass) { return nullptr; }
	UFunction* Fn = OwnerClass->FindFunctionByName(FuncName);
	if (!Fn) { UE_LOG(LogBPParserTestGen, Warning, TEXT("SpawnCallArrayFunc: %s not found on %s"), *FuncName.ToString(), *OwnerClass->GetName()); return nullptr; }
	UK2Node_CallArrayFunction* N = BeginNode<UK2Node_CallArrayFunction>(G);
	if (!N) { return nullptr; }
	N->SetFromFunction(Fn);
	FinalizeNode(N, X, Y);
	return N;
...
UK2Node_SpawnActorFromClass* FBPGen::SpawnActorNode(UEdGraph* G, UClass* ActorClass, int32 X, int32 Y)
{
	...
	if (!N) { return nullptr; }
	// ConstructObjectFromClass-derived nodes touch pins inside PostPlacedNewNode(),
	// so pins MUST be allocated first (the generic FinalizeNode order would assert).
	N->NodePosX = X;
	N->NodePosY = Y;
	N->AllocateDefaultPins();
	N->PostPlacedNewNode();
	...
	// SpawnTransform is an AutoCreateRefTerm (by-ref) pin and must have a real input,
	// otherwise the blueprint fails to compile ("must connect an input"). Feed a MakeTransform.
	if (UEdGraphPin* XformPin = FindPin(N, TEXT("SpawnTransform"), EGPD_Input))
	{
		if (XformPin->LinkedTo.Num() == 0)
		{
			K2()->SetPinAutogeneratedDefaultValueBasedOnType(XformPin);
			UK2Node_CallFunction* MakeXf = SpawnCallFunc(G, UKismetMathLibrary::StaticClass(), FName("MakeTransform"), X - 320, Y + 220);
			if (MakeXf)
			{
				if (UEdGraphPin* XfOut = FindPin(MakeXf, TEXT("ReturnValue"), EGPD_Output))
				{
					Connect(XfOut, XformPin);
				}
			}
		}
	}
...
UK2Node_CreateDelegate* FBPGen::SpawnCreateDelegate(UEdGraph* G, FName FunctionName, int32 X, int32 Y)
{
	...
	if (!N) { return nullptr; }
	FinalizeNode(N, X, Y);
	// NOTE: the function is bound by the caller AFTER the output delegate pin is wired,
	// because CreateDelegate needs the delegate signature from the connected pin first.
	(void)FunctionName;
	return N;
}
